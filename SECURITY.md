...
CI는 PR, `main` push, 매주 월요일 09:00 KST에 `npm run audit:ci`로 high·critical 취약점을
차단합니다. 수동 실행도 지원합니다. 검사기는 `npm audit --json`의
...
두 예외는 2026-09-30 종료 시각에 자동 만료되며, 그 전에 수정 릴리스 확인과 제거 여부를
재검토하지 않으면 CI가 실패합니다.
